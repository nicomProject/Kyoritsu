...
const PrintUtil = {
    print: function (params) {
        ...
        const div = modal.querySelector('.modal-content');
        ...
        win.document.write('<link rel="stylesheet" href="/static/css/lib/bootstrap/5.1.3/bootstrap.min.css">' +
            '<link rel="stylesheet" href="/static/css/admin/template/soft-ui-dashboard.min.css">' +
            '<link rel="stylesheet" href="/static/css/ezfind/style.css">' +
            '<link rel="stylesheet" href="/static/css/lib/leaflet/1.9.4/leaflet.css">' +
            '<link rel="stylesheet" href="/static/css/lib/tabulator/5.4/tabulator_semanticui.css">' +
            '<link rel="stylesheet" href="/static/css/lib/three-dots/three-dots.min.css">' +
            '<link rel="stylesheet" href="/static/css/lib/three-dots/_variable.css">' +
            '<link rel="stylesheet" href="/static/fonts/notokr/notokr.css">' +
            '<script type="text/javascript" src="/static/js/lib/jquery/3.5.1/jquery.min.js"></script>' +
            '<script type="text/javascript" src="/static/js/admin/components/modal.js"></script>' +
            '<script type="text/javascript" src="/static/js/ezfind/modal/eachLocation.js"></script>' +
            '<script type="text/javascript" src="/static/js/ezfind/modal/location.js"></script>');
       ...
       /* html2canvas(win.document.body, {
            useCORS: true, // CORS 관련 설정
        }).then(canvas => {
            const imgData = canvas.toDataURL('image/png');
            const img = new Image();
            img.src = imgData;

            // 이미지를 생성한 후 적절한 위치에 추가
            win.document.body.appendChild(img);
        });*/
        // 이미지 추가 후 프린트 및 팝업 닫기
        setTimeout(function () {
            win.print();
            win.close();
        }, 250);
    ...
    callNo: function (params) {
        const id = params.id || '';
        const title = params.title || '청구기호 출력 페이지';
        ...
        win.document.write('<link rel="stylesheet" href="/static/css/lib/bootstrap/5.1.3/bootstrap.min.css">' +
            ...
            '<script type="text/javascript" src="/static/js/ezfind/modal/eachLocation.js"></script>' +
            '<script type="text/javascript" src="/static/js/ezfind/modal/location.js"></script>' +
            '<script type="text/javascript" src="/static/js/ezfind/modal/callNo.js"></script>');
        win.document.write('</head><body>');
        win.document.write(div.innerHTML);
        win.document.write('</body></html>');
        win.document.close();
...
const MarkerUtil = {
    footer: {
        info: (type) => {
            const infoHash = {
                'antenna': 'btn-danger bg-gradient-danger',
                'shelf': 'btn-primary bg-gradient-primary',
                'desk': 'hide'
            };
            const moveHash = {
                'antenna': 'btn-secondary',
                'shelf': 'btn-secondary bg-gradient-secondary',
                'desk': 'btn-warning bg-gradient-warning'
            }
            return `<div class="tippy-footer">
                    <button type="button" class="btn ${type && infoHash[type] ? infoHash[type] : 'btn-primary bg-gradient-primary'}" role="action" data-action="mod">정보 수정</button>
                    <button type="button" class="btn ${type && moveHash[type] ? moveHash[type] : 'btn-secondary bg-gradient-secondary'}" role="action" data-action="location">위치 이동</button>
                </div>`;
        },
        move: (type) => {
            const saveHash = {
            ...
            return `<div class="tippy-footer">
                    <button type="button" class="btn ${type && saveHash[type] ? saveHash[type] : 'btn-primary bg-gradient-primary'}" role="action" data-action="save">위치 저장</button>
                    <button type="button" class="btn ${type && restoreHash[type] ? restoreHash[type] : 'btn-secondary bg-gradient-secondary'}" role="action" data-action="restore">원 위치로</button>
                </div>`;
        },
    },
    desk: {
        title: {
            saveLocation: '검색대 위치 저장',
            moveLocation: '검색대 위치 이동',
        },
        section: (data) => {
            return `<div class="tippy-title">[검색대]</div>
                    `;
        },
        func: (data) => {
            AjaxUtil.requestBody({
                ...
                data: {
                    key: data.room,
                    deskX: data.cx.toFixed(1),
                    deskY: data.cy.toFixed(1)
                },
                successMessage: '검색대 위치가 성공적으로 수정되었습니다.'
            });
        }
    },
    shelf: {
        title: {
            saveLocation: '서가 위치 저장',
            moveLocation: '서가 위치 이동'
        },
        section: (data) => {
            return `<div class="tippy-title">[서가 정보]</div>
        ...
        func: (data) => {
            AjaxUtil.requestBody({
                ...
                data: {
                    room: data.room,
                    no: data.no,
                    posX: data.cx.toFixed(1),
                    posY: data.cy.toFixed(1)
                },
                success: function (data) {
                    if (data.code === 200) {
                        Alert.success({text: '서가 위치가 성공적으로 수정되었습니다!'}, function () {
                            const table = Tabulator.findTable('#shelfTable')[0];
                            table.replaceData();
                        });
                    } else {
    ...
    antenna: {
        title: {
            saveLocation: '안테나 위치 저장',
            moveLocation: '안테나 위치 이동'
        },
        section: (data) => {
            return `<div class="tippy-title">[안테나 정보]</div>
                    <div class="tippy-body">
                        등록번호: ${data.serial}<br>
                        안테나명: ${data.name}<br><br>
                    </div>`
        },
        func: (data) => {
            AjaxUtil.requestBody({
                ...
                success: function (data) {
                    if (data.code === 200) {
                        Alert.success({text: '안테나 위치가 성공적으로 수정되었습니다!'}, function () {
                            const table = Tabulator.findTable('#antennaTable')[0];
                            table.replaceData();
                        });
                    } else {
                        Alert.danger({text: data.desc});
    ...
    time: {
        section: (data) => {
            return `<div class="tippy-title">[발자국 정보]</div>
                    <div class="tippy-body">
                        <i class="fas fa-clock"></i> ${data.logDate}<br>
                        <i class="fas fa-broadcast-tower"></i> 안테나: ${data.antenna}<br><br>
                    </div>`
        }
    }
};
...
class MapUtil {
    /**
     * MapUtil 클래스 생성
     * @param {string} id Container ID (DOM ID)
     * @param data 데이터
     * @param config 환경설정
     * @param {*} data.room 자료실 정보
     * @param {Array<*>} data.shelfs 서가 목록
     * @param {Array<*>} data.antennas 안테나 목록
     * @param {Array<*>} data.timeline 타임라인 목록
     * @param {Array<*>} data.books 도서 목록
     * @param {boolean} config.editable 수정 권한이 있는지 여부
     * @param {boolean} config.switch Switch 기능이 있는지 여부
     */
    constructor(id, data, config) {
        ...
        this.times = data.timeline || [];
        this.books = data.books || [];
        this.editable = config.editable && true;
        this.switch = config.switch && true;
    }
    ...
    initArea() {
        ...
        // 저장된 image가 없으면 전체 화면
        if (!room.imageName || room.imageName === '') {
            image.attr('width', '100%')
                .attr('height', '100%')
        } else {
            image.attr('xlink:href', '/api/room/image?key=' + room.key)
                .on('error', function (e) {
                    image.attr('xlink:href', '/static/images/banner/noimage.png')
                });
        }

        this.deskCircle = svg.selectAll("image.marker-desk")
            .data([{
                room: room.key,
                posX: room.deskX,
                posY: room.deskY,
                ...
                width: 20,
                height: 20,
                url: '/static/images/marker/search.png'
            }])
            .enter()
            .append("image")
            .attr('class', 'marker marker-desk')
            .attr('href', d => d.url)
            .attr('data-room', d => d.room)
            .attr('data-marker', 'desk')
        ...
        const zoom = d3.zoom()
            ...
            .on('zoom', function (e) {
                ...
                $zoom.text('x ' + transform.k.toFixed(1))
    ...
    processArea() {
        ...
        this.infoTip = this.infoTip || [];
        this.infoTip.forEach(tip => {
            tip.destroy();
        });
        ...
        svg.selectAll(".marker").each(function (d, i) {
            ...
            that.infoTip.push(that.createInfoTip(element, data, that.editable));
        });

        // 클릭시 상단에 보여지도록 동작
        svg.selectAll('.marker').on('mouseover', function (e) {
            this.parentNode.appendChild(this);
        });
        svg.selectAll('.marker').on('click', function (e) {
            this.parentNode.appendChild(this);
        });

        if (that.switch) {
            const mapSwitch = $('.room-switch input[role="switch"]');
            if (mapSwitch.length === 0) return;

            mapSwitch.each(function (e) {
                const target = this.dataset.switch;
                const checked = this.checked;
                d3.selectAll(`.marker-${target}`)
                    .attr(`class`, checked ? `marker marker-${target}` : `marker marker-${target} marker-hide`);
            });
    ...
    drawShelf() {
        ...
        const shelfData = shelfs.map(shelf => {
            return Object.assign({}, shelf, {
                cx: shelf.posX, cy: shelf.posY, width: 20, height: 20, url: '/static/images/marker/shelf.png'
            });
        });

        this.shelfCircle && this.shelfCircle.remove();
        this.shelfCircle = svg.selectAll("image.marker-shelf")
            .data(shelfData)
            .enter()
            .append("image")
            .attr('class', 'marker marker-shelf')
            .attr('data-marker', 'shelf')
            .attr("data-room", d => d.room)
            .attr("data-no", d => d.no)
            .attr('href', d => d.url);
    }
    ...
    drawBook() {
        ...
        const bookData = [];
        books.forEach(book => {
            bookData.push(Object.assign({}, book, {
                cx: book.posX,
                cy: book.posY,
                width: 20,
                height: 20,
                url: '/static/images/marker/original.png',
                tooltip: '원위치'
            }));
            bookData.push(Object.assign({}, book, {
                cx: book.nowX,
                ...
                width: 20,
                height: 20,
                url: '/static/images/marker/position.png',
                tooltip: '현위치'
            }));
        });

        this.bookCircle && this.bookCircle.remove();
        this.bookCircle = svg.selectAll("image.marker-book")
            .data(bookData)
            .enter()
            .append("image")
            .attr('class', 'marker marker-book')
            .attr('data-marker', 'book')
            .attr("data-room", d => d.room)
            .attr("data-no", d => d.barcode)
    ...
    drawAntenna() {
        ...
        // 안테나 데이터 및 Circle 생성
        const antennaData = antennas.map(antenna => {
            return Object.assign({}, antenna, {
                cx: antenna.posX, cy: antenna.posY, width: 20, height: 20, url: '/static/images/marker/antenna.png'
            });
        });

        this.antennaCircle && this.antennaCircle.remove();
        this.antennaCircle = svg.selectAll("image.marker-antenna")
            .data(antennaData)
            .enter()
            .append("image")
            .attr('class', 'marker marker-antenna')
            .attr('data-marker', 'antenna')
    ...
    /**
     * [MapUtil] 안테나 목록 업데이트
     * @param {Array<>} antennas 서가 목록
     */
    updateAntenna(antennas) {
    ...
    drawTimeline() {
        ...
        // 안테나 데이터 및 Circle 생성
        const timeData = times.map(e => {
            return Object.assign({}, e, {cx: e.posX, cy: e.posY, radius: 4});
        });

        this.timeCircle = svg.selectAll("circle.marker-time")
            .data(timeData)
            .enter()
            .append("circle")
            .attr('class', 'marker marker-time')
            .attr('data-marker', 'time')
            .attr("data-serial", d => d.serial);

        this.timeLine = svg.selectAll("path.line-time")
            .data(timeData)
    }
    ...
    subscribeLocation(action) {
        ...
        image.on('click', function (e) {
            ...
            that.position = {x: that.xScale(x).toFixed(1), y: that.yScale(y).toFixed(1)};
            action(that.position);
        });
    }
    ...
    scaling() {
        ...
        const area = $('#map svg');
        const node = $('#map image');

        let [widthArea, heightArea] = [area.width(), area.height()];
        let [widthImage, heightImage] = [node.width(), node.height()];
        let xScale = d3.scaleLinear().domain([0, widthImage]).range([0, room.maxX]);
        let yScale = d3.scaleLinear().domain([0, heightImage]).range([0, room.maxY]);

        this.xScale = xScale;
        this.yScale = yScale;

        if (heightImage * widthArea > widthImage * heightArea) {
        ...
        // Set circle attributes based on the data
        this.shelfCircle
            .attr('x', d => xScale.invert(d.cx) - d.width / 2)
            .attr("y", d => yScale.invert(d.cy) - d.height / 2)
        ...
        this.timeCircle
            .attr('cx', d => xScale.invert(d.cx))
            .attr("cy", d => yScale.invert(d.cy))
            .attr("r", d => d.radius)
            .attr("fill", d => d.fill);

        const circleDrag = d3.drag()
            .on('drag', function (e) {
                that.moveTip && that.moveTip.destroy();

                const marker = d3.select(this);
                const tag = marker.node().tagName;

                marker.style('cursor', 'grabbing');

                if (tag === 'circle') {
                    marker.attr('cx', d => {
                        let cx = e.x > widthImage ? widthImage : e.x;
                        if (cx < 0) cx = 0;
                        d.cx = xScale(cx);

                        return cx;
                    }).attr('cy', d => {
                        let cy = e.y > heightImage ? heightImage : e.y;
                        if (cy < 0) cy = 0;
                    ...
                    marker.attr('x', d => {
                        let cx = e.x - d.width / 2 > widthImage ? widthImage : e.x - d.width / 2;
                        if (cx < 0) cx = 0;
                        d.cx = xScale(cx);

                        return cx;
                    }).attr('y', d => {
                        let cy = e.y - d.height / 2 > heightImage ? heightImage : e.y - d.height / 2;
                        if (cy < 0) cy = 0;
            ...
            .on('end', function (e) {
                const obj = d3.select(this).style('cursor', 'grab');
                const data = obj.datum();

                that.moveTip && that.moveTip.destroy();
                that.moveTip = that.createMoveTip(this, data);
            });

        svg.selectAll('circle.draggable')
            .call(circleDrag);

        svg.selectAll('image.draggable')
            .call(circleDrag);
    }
     ...
     * @param data 마커 데이터
     * @param {boolean} editable 수정 가능 여부
     * @returns {*}
     */
    createInfoTip(element, data, editable = false) {
        let type = element.dataset.marker || 'general';

        const markerDef = MarkerUtil[type];
        if (!markerDef) {
        ...
        let content = markerDef.section(data);
        // 수정 권한 설정, 검색대는 가능
        if (editable || type === 'desk') {
            content += MarkerUtil.footer.info(type);
        }
        ...
        return tippy(element, {
            ...
            onShown(instance) {
                ...
                popper.find('.btn[role="action"]').on('click', function (e) {
                    const action = this.dataset.action;
                    instance.hide();

                    if (action === 'mod') {
                        ParamManager.show(type, action, data);
                    }
                    // 위치 이동
                    else if (action === 'location') {
                        Alert.warning({
                            title: markerDef.title.moveLocation,
                            text: `화면에서 반짝이는 요소의 위치를 이동해 주세요!`
                        }, function () {
                            d3.selectAll('circle').classed('draggable', false);
                            d3.selectAll('circle').classed('active', false);
                            reference.addClass('active draggable');
                        })
                    }
                });
            },
            onHide(instance) {
                const obj = $(instance.popper);
                obj.find('.btn[role="action"]').unbind();
            }
        });
    }
    ...
    createMoveTip(element, data) {
        ...
        const marker = d3.select(element);
        let content = markerDef.section(data) + MarkerUtil.footer.move(type);
        ...
        return tippy(element, {
            ...
            onShown(instance) {
                ...
                popper.find('.btn[role="action"]').on('click', function (e) {
                    ...
                    if (action === 'save') {
                        Alert.confirm({
                            title: markerDef.title.saveLocation,
                            text: '위치를 저장하시겠습니까?'
                        }, function (result) {
                        ...
                        reference.removeClass('active draggable');
                        that.moveTip.destroy();
                    }
                });
            },
            onHide(instance) {
