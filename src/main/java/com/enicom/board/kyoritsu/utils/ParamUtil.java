...
public class ParamUtil {
	...
	public static boolean checkRequired(HttpServletRequest request, String[] keys) {
		return checkRequired(convertRequestToMap(request), keys);
	}

	public static String checkRequiredMessage(String[] keys) {
		return "필수 파라미터가 누락되었습니다. 필수 파라미터(" + String.join(", ", keys) + ")";
	}
	...
	public static Map<String, Object> convertRequestToMapWithKeys(HttpServletRequest request, String[] keys) {
		Map<String, Object> paramMap = convertRequestToMap(request);
		for (String key : keys) {
			if (paramMap.get(key) == null)
				paramMap.put(key, "");
		}
